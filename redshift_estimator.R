...
if(run_locally){
  ...
  #gen_dat <- read.table("test.txt", header = TRUE, row.names = 1)
  do_mice = F
  supmodel = F
} else {
...
if(do_mice){
  set.seed(1)
  md.pattern(gen_dat, rotate.names = T)
  mice_model <- mice(data = gen_dat
                     ,m = 20
                     ,method = 'midastouch'
                     ,printFlag = F)
  gen_dat <- complete(mice_model,20)
}else{
  gen_dat<-na.omit(gen_dat)
}
...
if(do_mice){
  set.seed(1)
  mice_model <- mice(data = gen_dat_preds
                     ,m = 20
                     ,method = 'midastouch'
                     ,printFlag = F)
  gen_dat_preds <- complete(mice_model,20)
  #md.pattern(complete(mice_model,20), rotate.names = T)
}else{
...
if(do_mice){
  #mice(gen_dat_preds)
  set.seed(1)
  mice_model <- mice(data = gen_dat_errs
                     ,m = 20
                     ,method = 'midastouch'
                     ,printFlag = F)
  gen_dat_errs <- complete(mice_model,20)
}else{gen_dat_errs<-na.omit(gen_dat_errs)
}
#is.na(gen_dat_preds)
...
gen_dat <- cbind(gen_dat_preds, gen_dat_errs[rownames(gen_dat_preds),])
...
#   par(mar=c(10,6,2,2))
#   boxplot(t(ordered_GRB_list[,2 ]), names = ordered_GRB_list[,1 ]
#           ,ylab = "",xlab = ""
