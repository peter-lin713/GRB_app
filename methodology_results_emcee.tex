...
%%
%% NOTE: the linear-z formula rerun (Eq.~\ref{eq:winner}) and the baseline
%% SuperLearner results (Sect.~\ref{sec:res-cv}) below are final on the
%% corrected 266-burst sample. The augmented SuperLearner configuration
%% (Sect.~\ref{sec:sl}) has not yet been rerun on this corrected sample; its
%% numbers from the prior pipeline are cited once, in prose, purely as an
%% architecture-comparison data point, and are not tabulated as current results.
%% ============================================================================
\documentclass{aa}
...
{Cross-calibrating optical- and X-ray-domain plateau parameters recovers a
 ...
 training samples. We also find that reproducing the parent paper's
 single-split diagnostic directly, rather than an aggregate across splits, is
 essential to correctly interpret which formulae the search actually favours.}
...
\begin{figure*}
  \centering
  \includegraphics[width=0.95\textwidth]{Plot_Output/scatter_matrix_emcee_v2_native_vs_optical.png}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{Plot_Output/emcee_calibration_v2/emcee_optical_vs_xray.png}
...
(Sects.~\ref{sec:lasso}--\ref{sec:mc}) is currently being re-run on it under the
linear-$z$ formula search; we report its results alongside the emcee-based ones in
a forthcoming update once that search completes.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{Plot_Output/fundamental_plane_4d_emcee_v2_current.png}
...
$\log \Fa$, $\log$~PeakFlux, $\log \NH$, photon index, $\log \Ta$, $\alpha$, and
$\log T_{90}$ (Fig.~\ref{fig:lasso}), matching the seven-feature choice of the parent
paper.

\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{runs/emcee_v2_cleaned_linearz_5pct/Plot_Output/LassoFeatures.png}
  \caption{Mean absolute LASSO coefficient of each candidate feature over 100
    resamples. The seven highest-ranked features (above) are retained for formula
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{formula_results/plots/fig6_paper_style_emcee_LINEARZ_run.png}
...
configurations. The \emph{baseline} configuration uses a broad learner library
(elastic net, gradient-boosted trees, CART, MARS, bagged trees, neural network,
random forests, stepwise regressions, and the tuned GAM/GLM/Bayesian-GLM learners
built from Eq.~\ref{eq:winner}), imputes missing values with multiple imputation by
chained equations \citep[MICE;][]{vanbuuren2011} using the predictive-mean-matching
``midastouch'' method with $m=20$ (predictors and errors imputed separately), stacks
by non-negative least squares, and is cross-validated with 10fCV repeated 10 times.

The \emph{augmented} configuration adds several components aimed at exploiting the
optical/X-ray heterogeneity. Missing predictors are co-imputed with
\texttt{missForest} \citep{stekhoven2012missforest} jointly with an unlabelled
generalisation pool (which never sees the redshift). Domain membership is encoded
through a Daum\'e-style augmentation \citep{daume2007}: an \texttt{is\_optical}
indicator plus scaled per-domain copies of the predictors ($c=0.25$). The learner
library additionally includes a band-interaction GLM (with \texttt{is\_optical}
interacting with the plateau terms) and a boosted additive model
...
(Sects.~\ref{sec:cuts}, \ref{sec:mest}); the numbers below are the final baseline
results on the corrected 266-burst sample. The augmented configuration
(Sect.~\ref{sec:sl}) has not yet been rerun on this corrected sample and is
therefore omitted from the table below; see the note at the end of
Sect.~\ref{sec:res-cv}.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{runs/emcee_v2_cleaned_linearz_5pct/Plot_Output/z_pred_v_obs_log_without_catOutl_correlation_plot.png}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{runs/emcee_v2_cleaned_linearz_5pct/Plot_Output/z_pred_v_obs_linear_without_catOutl_correlation_plot.png}
...
The augmented configuration of Sect.~\ref{sec:sl} has not yet been rerun on this
corrected sample; on the prior (pre-correction) pipeline it attained a comparable
but not superior $\logz$ correlation to the baseline ($r=0.641$ versus $0.646$),
suggesting the additional missForest co-imputation, domain-augmentation, and
alternative stacking machinery does not by itself yield a measurable gain over the
baseline ensemble. We defer a rerun of the augmented configuration on the corrected
sample to future work and do not report its numbers further here.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{runs/emcee_v2_cleaned_linearz_5pct/Plot_Output/mc_pred_vs_obs_log10_inside.png}
...
The augmented SuperLearner configuration (Sect.~\ref{sec:sl}) has not yet been
rerun on the corrected sample, and on the prior pipeline its additional
machinery (missForest co-imputation, Daum\'e-style domain augmentation,
alternative stacking) bought no measurable improvement over the baseline
(Sect.~\ref{sec:res-cv}). This null result is consistent with
...
optical-plateau population is positioned to fill.
...
\item The augmented SuperLearner configuration has not yet been rerun on the
  corrected sample; its prior null result relative to the baseline is
  consistent with sample size, rather than ensemble architecture, being the
  binding constraint on performance at the currently available LGRB sample
  sizes.
