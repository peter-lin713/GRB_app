...
if(run_locally){
  raw_xray_data <- read.csv("superlearner_training_emcee_v2_native_opt.csv", header = TRUE, row.names = 1)
} else {
...
# alongside, not in place of, the standard emcee-projected Alpha/Beta/Fa/Ta,
...
# manual additions from 4D-plot review preserved across reruns). Deterministic
# (no randomness), so safe even if GAM and GLM happen to run concurrently.
source("../../generate_nonnegotiable_outliers.R")
.outliers <- update_nonnegotiable_outliers(raw_xray_data, out_dir = ".", method = "emcee_v2_native_opt")
...
N_SPLITS <- 100
CKPT_DIR <- "checkpoints_GLM"  # separate from the GAM search's "checkpoints" --
                                # same data/formula list, independent search+tally,
                                # per the paper's "determined independently" (Sec 4.2.3)
...
for (s in 1:N_SPLITS) {
  ...
  scores <- foreach(j = 1:n_formula, .combine = rbind,
    ...
    ok <- tryCatch({
      ...
      for (i in seq_along(folds)) {
        g <- stats::glm(fm, data = TrainSet[-folds[[i]], ], family = gaussian())
  ...
  test_eval <- t(sapply(cand, function(j) {
    tryCatch({
      g <- stats::glm(all_formula[[j]], data = TrainSet, family = gaussian())
      p <- predict(g, TestSet)
      z_pred <- 10^p - 1
...
write.csv(freq, "formula_win_frequency_GLM.csv", row.names = FALSE)
cat("Splits completed:", length(files),
    "| top formulas written to formula_win_frequency_GLM.csv\n")
print(head(freq, 10))
...
if (length(files) == N_SPLITS) {
  ...
  cat("Most-frequent winning GLM formula (", freq$wins[1], "/", N_SPLITS, "splits):\n")
  print(Formula_for_outlier)
  writeLines(deparse(Formula_for_outlier), "Formula_for_outlier_GLM.txt")
  ...
  cat("M-estimator outlier cut (GLM formula):", length(removed_rows), "of", nrow(SqrData),
      "GRBs removed (weight <=", round(weight_threshold, 4), ")\n")
  writeLines(removed_rows, "removed_outliers_GLM.txt")

  final_cut <- raw_xray_data[rownames(raw_xray_data) %in% kept_rows, ]
  write.csv(final_cut, "final_outliers_removed_GLM.csv")
  cat("Final outlier-removed data saved:", nrow(final_cut), "GRBs -> final_outliers_removed_GLM.csv\n")
} else {
