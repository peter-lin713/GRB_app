...
if(run_locally){
  raw_xray_data <- read.csv("superlearner_training_emcee_v2_recovered.csv", header = TRUE, row.names = 1)
} else {
...
# ---- Manual outlier confirmation checkpoint --------------------------------
# Per the "Pipeline for Data Combination" doc: GRBs failing the non-negotiable
# cuts (Alpha>3, Beta>2, log10T90>6, PhotonIndex<0) get REMOVED COMPLETELY --
# not NA'd/imputed. candidate_outliers_emcee_v2.csv lists everything the
# automatic numeric criteria flag; confirmed_outliers_to_drop.txt is the
# actual drop list -- pre-populated with those same candidates, but meant to
# be edited by hand after reviewing the Alpha/Beta/log10Fa/log10Ta 4D plots
# (physically-implausible GRBs the numeric cuts miss can be added here too).
# Edit confirmed_outliers_to_drop.txt, then (re-)run this script.
confirm_file   <- "confirmed_outliers_to_drop.txt"
candidate_file <- "candidate_outliers_emcee_v2.csv"

to_drop <- if (file.exists(confirm_file)) readLines(confirm_file) else character(0)
to_drop <- trimws(to_drop)
to_drop <- to_drop[nzchar(to_drop) & !grepl("^#", to_drop)]

candidates <- if (file.exists(candidate_file)) {
  read.csv(candidate_file, stringsAsFactors = FALSE)
} else {
  data.frame(GRB = character(0), reasons = character(0))
}
reason_for <- setNames(candidates$reasons, candidates$GRB)
...

{
  ...
  # 1000-split run: reuse the exact O1/O2 lists already committed from the
  # original 100-split run (same LASSO top-7 variables, same 16,510-formula
  # candidate space) rather than letting an unseeded LASSO re-ranking
  # possibly pick a different top-7 -- this run is meant to test "same
  # formulas, more splits," not "different formulas AND more splits."
  if(O2){
    if (!file.exists("O2_formula_list")) write.csv(formula_list,paste0("O2_formula_list"))
  }else{
    if (!file.exists("O1_formula_list")) write.csv(formula_list,paste0("O1_formula_list"))
  }
}
...
#formula_list
if (!file.exists("SmoothedO1_formula_list")) write.csv(formula_list,paste0("SmoothedO1_formula_list"))
...
MyLaptop = T
if (MyLaptop) {
  slaves <- detectCores() - 1  # OT's concurrent run was dropped; use all available cores
...
N_SPLITS <- 3  # SMOKE TEST ONLY -- the supercomputer package sets this to 1000
