...
if(run_locally){
  raw_xray_data <- read.csv("superlearner_training_hardzero_domain_v2.csv", header = TRUE, row.names = 1)
} else {
...
# Hard-zero domain scheme (no calibration/regression fit at all): each GRB
# carries EITHER a real core (X-ray-scale) value XOR a real _opt (optical-
# native-scale) value for each of Fa/Ta/Alpha/Beta -- the other is a
# structural 0, not NA/imputed. Lets the model learn separate domain-specific
# slopes directly, same spirit as the Daume is_optical columns but hard
# (0/real) rather than soft (value*is_optical*0.25).
...
# confirmed_outliers_to_drop.txt fresh every run.
source("../../generate_nonnegotiable_outliers.R")
.outliers <- update_nonnegotiable_outliers(raw_xray_data, out_dir = ".", method = "hardzero_domain_v2")
...
if (length(files) == N_SPLITS) {
  # Fallback-to-next-ranked-formula: formulas that interact two _opt columns
  # together (or their squares) are exactly collinear here -- all _opt columns
  # share the same zero/nonzero row pattern (xray-only + shared GRBs are 0/NA,
  # optical-only are real) -- so rlm() can throw 'x is singular'. Walk down the
  # ranked list until one fits cleanly instead of hand-fixing this after a crash.
  require(MASS)
  M_est <- NULL
  for (.i in seq_len(nrow(freq))) {
    .fm <- tryCatch(as.formula(freq$formula[.i]), error = function(e) NULL)
    if (is.null(.fm)) next
    .fit <- tryCatch(MASS::rlm(.fm, data = SqrData, method = "M", maxit = 50), error = function(e) e)
    if (inherits(.fit, "rlm")) {
      cat("M-estimator formula: rank", .i, "(formula_idx", freq$formula_idx[.i],
          ", wins", freq$wins[.i], "/", N_SPLITS, ")\n")
      Formula_for_outlier <- .fm
      M_est <- .fit
      break
    } else {
      cat("M-estimator formula rank", .i, "(idx", freq$formula_idx[.i], ") FAILED --",
          conditionMessage(.fit), "-- trying next-ranked formula\n")
    }
  }
  stopifnot("no ranked formula fit rlm() without error" = !is.null(M_est))
