...
if(run_locally){
  raw_xray_data <- read.csv("superlearner_training_ot_v3_recovered.csv", header = TRUE, row.names = 1)
} else {
...
# cuts (Alpha>3, Beta>2, log10T90>6, PhotonIndex<0, err>value) get REMOVED
# COMPLETELY -- not NA'd/imputed. candidate_outliers_ot_v3.csv lists everything
# the automatic numeric criteria flag; confirmed_outliers_to_drop.txt is the
# actual drop list -- pre-populated with those same candidates, editable after
# reviewing the 4D fundamental-plane plots. Edit confirmed_outliers_to_drop.txt,
# then (re-)run this script.
confirm_file   <- "confirmed_outliers_to_drop.txt"
candidate_file <- "candidate_outliers_ot_v3.csv"
...
MyLaptop = T
if (MyLaptop) {
  slaves <- 6  # OT v3 recovered's redo is running concurrently on this
               # machine (corrected outlier cut); split the 12 cores evenly
