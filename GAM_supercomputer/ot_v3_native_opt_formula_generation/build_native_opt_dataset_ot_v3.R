#!/usr/bin/env Rscript
#' build_native_opt_dataset_ot_v3.R -- OT v3 analogue of
#' combine_opt_xray_emcee/build_native_opt_dataset.R. ADDS the real, native
#' optical-domain measurements (Alpha_opt/Beta_opt/log10Faopt/log10Taopt from
#' the optical catalog, renamed *_native here) as four EXTRA candidate columns,
#' alongside -- not replacing -- the existing OT-projected
#' Alpha/Beta/log10Fa/log10Ta. Only the 73 optically-sourced GRBs
#' (is_optical == 1) have a real value for these; the 223 X-ray-native GRBs
#' get NA, imputed downstream by MICE the same way any other missing
#' predictor is (75% missingness on this column, so treat that imputation as
#' weak/exploratory, not a strong signal).
#'
#' This lets LASSO's own ranking decide whether the native-optical version of
#' a variable carries more predictive signal than the standard (OT-fusion)
#' version for the GRBs that have both -- rather than us presupposing an
#' answer by overwriting one with the other.

setwd("/Users/petelin/Desktop/test/GRB_app")

d   <- read.csv("Data/superlearner_training_ot_v3_recovered.csv", row.names = 1)
...
write.csv(d, "Data/superlearner_training_ot_v3_native_opt.csv")
cat("Wrote Data/superlearner_training_ot_v3_native_opt.csv (", nrow(d), "GRBs x", ncol(d),
