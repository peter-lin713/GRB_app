...
# Replaces the old NA-and-reimpute-only handling of log10NH<20/Beta>2/Gamma>3/
# Alpha>3/PhotonIndex<0 (this script predated generate_nonnegotiable_outliers.R
# and never applied the err>value(Alpha/Beta/log10Fa/log10Ta) cut at all;
# log10NH<20 and Gamma>3 were dropped as heuristics superseded by the cleaned
# pipeline's criterion, which does not include them).
source("../../generate_nonnegotiable_outliers.R")
.outliers <- update_nonnegotiable_outliers(raw_xray_data, out_dir = ".", method = "ot_v3_recovered")
...
MyLaptop = T  # rerunning locally with the corrected outlier cut (was
              # supercomputer-submitted before the fix); OT v3 cleaned's
              # search is also running on this machine, so throttle down
if (MyLaptop) {
  slaves <- 6
