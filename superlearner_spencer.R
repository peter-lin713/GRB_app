...
#'                              GRBs from the single canonical optical catalog,
#'                              Data/OnlyLGRBs_data_171_optical_processed_error-cut_MICE (1).csv
#'                              (already error-cut + MICE-imputed -- the same file
#'                              used everywhere else in the pipeline for combination
#'                              and overlap counting); drop short GRBs (log10T90 cut);
...
# is_optical: robust direct membership check (GRB not present in the X-ray-
# native catalog) -- NOT gated on how many features happen to already be
# missing in the input file. The original >=6-missing heuristic silently
# never fires once an upstream combination script has already filled these
# columns with placeholder values (exactly the case for our datasets), and
# needed an artificial nulling workaround to exercise at all. This matches
# the actual best-result campaign script's (cv_maxstack_v2.R) definition,
# which just checks catalog membership directly.
...
xray_ids <- character(0)
xray_cat_file <- "Data/Xray_data_with_redshift_V8-web-app_processed_filtered_MICE (1).csv"
if (file.exists(xray_cat_file)) {
  xr <- read.csv(xray_cat_file, stringsAsFactors = FALSE)
  xray_ids <- norm_grb(xr$GRB_Name)
}
raw_xray_data$is_optical <- as.integer(!norm_grb(raw_xray_data$GRB) %in% xray_ids)
cat("is_optical: flagged", sum(raw_xray_data$is_optical), "of", nrow(raw_xray_data),
    "GRBs as optical-only (not in X-ray-native catalog)\n")

optical_native <- NULL
opt_rows <- which(raw_xray_data$is_optical == 1)
opt_cat_file <- "Data/OnlyLGRBs_data_171_optical_processed_error-cut_MICE (1).csv"
if (length(opt_rows) > 0 && file.exists(opt_cat_file)) {
  opt_cat <- read.csv(opt_cat_file, stringsAsFactors = FALSE)
  m  <- match(norm_grb(raw_xray_data$GRB[opt_rows]), norm_grb(opt_cat$GRB))
  ok <- !is.na(m); opt_rows <- opt_rows[ok]; m <- m[ok]
  if (length(opt_rows) > 0) {
    # This file already carries log10T90 / log10Fluence / log10NH / log10PeakFlux
    # in the same converted units as raw_xray_data (verified against the older
    # optical_data.csv row by row: e.g. log10Fluence = log10(Fluence) - 7,
    # log10NH = log10(NH) + 21 are already applied) -- no further transform needed.
    raw_xray_data$log10T90[opt_rows]      <- opt_cat$log10T90[m]
    raw_xray_data$log10Fluence[opt_rows]  <- opt_cat$log10Fluence[m]
    raw_xray_data$PhotonIndex[opt_rows]   <- opt_cat$PhotonIndex[m]
    raw_xray_data$log10NH[opt_rows]       <- opt_cat$log10NH[m]
    raw_xray_data$log10PeakFlux[opt_rows] <- opt_cat$log10PeakFlux[m]
    raw_xray_data$Gamma[opt_rows]         <- opt_cat$Gamma[m]
    optical_native <- data.frame(logFa = opt_cat$log10Faopt[m], logTa = opt_cat$log10Taopt[m],
                                 Alpha = opt_cat$Alpha_opt[m], Beta = opt_cat$Beta_opt[m],
...
#' Physically-impossible values: remove the GRB entirely rather than nulling
#' the offending column and letting MICE paper over it -- a value outside the
#' plausible range is a data-quality problem with that GRB, not something an
#' imputer should reconstruct. Removed GRBs are recorded with their reason.
bad_reasons <- character(nrow(features_for_mice_preds))
flag <- function(cond, label) {
  hit <- which(cond & bad_reasons == "")
  bad_reasons[hit] <<- label
}
flag(features_for_mice_preds$log10NH < 20,                "log10NH<20")
flag(features_for_mice_preds$Beta > 2,                    "Beta>2")
flag(features_for_mice_preds$Gamma > 3,                   "Gamma>3")
flag(features_for_mice_preds$Alpha > 3,                    "Alpha>3")
flag(features_for_mice_preds$PhotonIndex < 0,              "PhotonIndex<0")
flag(is.infinite(features_for_mice_preds$log10PeakFlux),   "log10PeakFlux=Inf")
bad <- bad_reasons != ""
if (any(bad)) {
  removed_log <- data.frame(GRB = rownames(features_for_mice_preds)[bad],
                             reason = bad_reasons[bad])
  write.csv(removed_log, file.path(out_dir, "removed_feature_outliers.csv"), row.names = FALSE)
  cat("Removed", sum(bad), "GRB(s) for physically-impossible feature values",
      "(recorded in removed_feature_outliers.csv):\n")
  print(removed_log)
  raw_xray_data           <- raw_xray_data[!bad, ]
  features_for_mice_preds <- features_for_mice_preds[!bad, ]
}
...
#' The fixed Best_formula_*.txt formulas were tuned against a SEPARATE LASSO
#' ranking (GAM_analysis_8variables.R's own, on its own imputation/data). The
#' `lassovar` ranking above is freshly recomputed on THIS script's own imputed
#' data -- the two rankings are not guaranteed to agree, especially for
#' variables that sit near the top-7 cutoff. If the Sqr-term set below only
#' covered *this* ranking's top-7, a formula referencing a Sqr term for a
#' variable that fell just outside our top-7 would crash every learner fit
#' with "object '<var>Sqr' not found" -- this is exactly what broke the OT
#' run: log10T90 sits right at the rank-7/8 boundary depending on the exact
#' imputation draw, and it fell to rank 8 there while the fixed OT formula
#' still needs log10T90Sqr. Force in whichever variables the fixed formula
#' files actually reference, regardless of where our own ranking puts them.
formula_required_sqr_vars <- character(0)
for (.ff in c("Best_formula_GAM.txt", "Best_formula_GLM.txt")) {
  if (file.exists(.ff)) {
    .tbl <- tryCatch(read.table(.ff, stringsAsFactors = FALSE), error = function(e) NULL)
    if (!is.null(.tbl)) {
      .forms <- tryCatch(apply(as.matrix(.tbl[, ncol(.tbl)]), 1, as.formula), error = function(e) list())
      .vars  <- unique(unlist(lapply(.forms, all.vars)))
      formula_required_sqr_vars <- union(formula_required_sqr_vars,
                                          sub("Sqr$", "", grep("Sqr$", .vars, value = TRUE)))
    }
  }
}
sqr_vars <- union(head(lassovar, 7), intersect(formula_required_sqr_vars, lassovar))
if (!setequal(sqr_vars, head(lassovar, 7))) {
  cat("Sqr-term set adjusted to match fixed formula requirements:", paste(sqr_vars, collapse = ", "), "\n")
}

build_design <- function(feats) {
  X <- feats[rownames(GRBPred), lassovar, drop = FALSE]
  for (v in sqr_vars) X[[paste0(v, "Sqr")]] <- X[[v]]^2
...
SL.gamboost <- function(Y, X, newX, family, obsWeights, ...) {
  ...
  # No internal cvrisk() mstop-tuning: on this sample size that 5-fold inner CV
  # multiplied every outer-CV call into 6 full 300-iteration boosting fits
  # (checking all 10 smooth terms at every iteration), run serially
  # (papply = lapply) to avoid oversubscribing the outer fork-based
  # parallelism -- by far the slowest learner in the library, and the source
  # of ~400k repeated "df equal to rank of null space" warnings from
  # refitting degenerate small-sample P-spline bases. A fixed, smaller mstop
  # is a single fit; the outer SuperLearner CV already down-weights an
  # under/over-fit learner via its own stacking step, so precise internal
  # tuning here isn't needed for the ensemble to still use gamboost sensibly.
  fit <- mboost::gamboost(fm, data = df, control = mboost::boost_control(mstop = 100, nu = 0.1))
...
run_one_loop <- function(j) {
  ...
  for (i in fold_iters) {
    ...
    Pf_iso  <- vector("list", length(X_frames))  # P_frames[[f]] + outlier_score, per frame
    sc_tr_1 <- NULL                              # frame-1 scaling, reused for test_set/MC below
    prm  <- matrix(NA_real_, nrow(test_set), length(X_frames))
    for (f in seq_along(X_frames)) {
      Xtr_f <- X_frames[[f]][-folds[[i]], , drop = FALSE][keep_tr, , drop = FALSE]
      Xte_f <- X_frames[[f]][ folds[[i]], , drop = FALSE]

      # Isolation/anomaly feature: distance to the 10th-nearest neighbor in the
      # standardized raw-predictor space, fit on this fold's training rows only
      # and applied out-of-sample to test/holdout rows (no leakage). Matches
      # spencer's actual best-result harness (add_iso() in harness_prep.R) --
      # missing from the earlier version of this script.
      iso_cols <- intersect(pred_vars, colnames(Xtr_f))
      sc_tr    <- scale(Xtr_f[, iso_cols])
      Xtr_f$outlier_score <- FNN::knn.dist(sc_tr, k = 10)[, 10]
      sc_te    <- scale(Xte_f[, iso_cols],
                        center = attr(sc_tr, "scaled:center"),
                        scale  = attr(sc_tr, "scaled:scale"))
      Xte_f$outlier_score <- FNN::get.knnx(sc_tr, sc_te, k = 10)$nn.dist[, 10]
      if (f == 1) sc_tr_1 <- sc_tr

      Pf              <- P_frames[[f]]
      sc_pf           <- scale(Pf[, iso_cols],
                               center = attr(sc_tr, "scaled:center"),
                               scale  = attr(sc_tr, "scaled:scale"))
      Pf$outlier_score <- FNN::get.knnx(sc_tr, sc_pf, k = 10)$nn.dist[, 10]
      Pf_iso[[f]]      <- Pf
    ...
    #' test_set needs the same outlier_score column s9 was trained with (frame-1
    #' scaling); mc_predict()'s internal predict() call requires every training
    #' column to be present.
    iso_cols_mc <- intersect(pred_vars, colnames(test_set))
    sc_test_mc  <- scale(test_set[, iso_cols_mc],
                        center = attr(sc_tr_1, "scaled:center"),
                        scale  = attr(sc_tr_1, "scaled:scale"))
    test_set$outlier_score <- FNN::get.knnx(sc_tr_1, sc_test_mc, k = 10)$nn.dist[, 10]
    ...
    hp <- rowMeans(sapply(seq_along(fits), function(f) as.numeric(predict(fits[[f]], Pf_iso[[f]])$pred)))
...
# Persist the final model trained on the full training set. Needs the same
# outlier_score column the per-fold models were trained with -- self-
# referential k-NN distance here since there's no held-out set at this stage.
Predictors_final <- Predictors
iso_cols_final    <- intersect(pred_vars, colnames(Predictors_final))
sc_final          <- scale(Predictors_final[, iso_cols_final])
Predictors_final$outlier_score <- FNN::knn.dist(sc_final, k = 10)[, 10]

InsideCone <- read.csv(paste(addr, "Results_wo_catout", plotnames, ".csv", sep = ""), row.names = 1)
sl_model   <- SuperLearner(Y = Response, X = Predictors_final, family = gaussian(),
