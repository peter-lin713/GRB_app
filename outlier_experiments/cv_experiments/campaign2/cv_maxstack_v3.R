# cv_maxstack_v3.R — v2 + physical ratio features (FaTa, FluT90, AlBe). Session-2
# round-6 consolidation. Everything else identical to v2 (same seeds/folds) so the
# comparison to v2's logged numbers is paired.
...
VARS <- c(linear_vars, paste0(top7, "Sqr"), "is_optical", "FaTa", "FluT90", "AlBe")
...
for (d in seq_len(N_DRAWS)) {
  ...
  fd$FaTa   <- fd$log10Fa - fd$log10Ta        # plateau lum-duration (Dainotti)
  fd$FluT90 <- fd$log10Fluence - fd$log10T90
  fd$AlBe   <- fd$Alpha - fd$Beta
  frames[[d]] <- fd
}
