...
run_task <- function(t) {
  ...
  set.seed(260000 + 1000 * t$rep + t$fr)
  ...
  # full-train fit -> score seed-42 holdout (once per rep x frame; rep varies only RNG)
  fit_full <- try(suppressWarnings(SuperLearner(
    Y = Y[train_idx], X = X[train_idx, ], newX = X[holdout_idx, ], family = gaussian(),
    SL.library = LEAN, method = method.NNRidge(),
    cvControl = list(V = 5), verbose = FALSE)), silent = TRUE)
...
cat("\n== ROUND 23 PIPELINE-CONVENTION (seed-42 233/58) ==\n")
