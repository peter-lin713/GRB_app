...
"""
Combine optical and x-ray GRB data using MCMC (emcee) calibration.

Overlapping GRBs are used to fit a linear relationship between optical
and x-ray light-curve parameters (logFa, logTa, Alpha, Beta). Optical-only
GRBs are then projected to x-ray scale using the posterior medians.

Naming fix: x-ray IDs with no letter suffix (e.g. GRB050319) are treated as
the "A" event (GRB050319A) when matching against the optical catalog, which
always includes the suffix.

Input files (both CSV with named columns):
  optical_data.csv : GRB (index), z, logFa, logFaErr, logT_a, logTaErr,
                     Alpha, AlphaErr, Beta, betaErr, ...
  xray_data.csv    : GRB (index, prefixed "GRB..."), Redshift_crosscheck,
                     log10Fa, log10Ta, Alpha, Beta, log10FaErr, log10TaErr,
                     AlphaErr, BetaErr, T90, T90Err, log10Fluence, FluenceErr,
                     PhotonIndex, PhotonIndexErr, log10NH, log10PeakFlux, PeakFluxErr, Gamma
"""
...
DATA_DIR    = '/Users/peterlin/Desktop/test/GRB_app/Data'
OUT_DIR     = '/Users/peterlin/Desktop/test/GRB_app'
OPT_FILE    = f'{DATA_DIR}/optical_data.csv'
XRAY_FILE   = f'{DATA_DIR}/xray_data.csv'
OUT_FILE    = f'{DATA_DIR}/superlearner_training_emcee.csv'
PLOT_DIR    = f'{OUT_DIR}/Plot_Output/emcee_calibration'
CHAINS_FILE = f'{DATA_DIR}/emcee_chains.npz'   # cached chains; delete to rerun MCMC
...
opt_sel = opt_sel.rename(columns={
    ...
    'logT_a':  'logTa_opt',  'logTaErr': 'logTaErr_opt',
    'Alpha':   'Alpha_opt',  'AlphaErr': 'AlphaErr_opt',
    'Beta':    'Beta_opt',   'betaErr':  'BetaErr_opt',
})
...
xray = xray.rename(columns={
    'log10Fa':    'logFa_x',   'log10FaErr':  'logFaErr_x',
    'log10Ta':    'logTa_x',   'log10TaErr':  'logTaErr_x',
    'Alpha':      'Alpha_x',   'AlphaErr':    'AlphaErr_x',
    'Beta':       'Beta_x',    'BetaErr':     'BetaErr_x',
...
fig.suptitle('Emcee calibration: optical vs X-ray (overlapping GRBs)\n'
...
for ax, (param, (xc, xe, yc, ye)) in zip(axes.flat, params_map.items()):
    ...
    flat = r['flat']
    m_med, b_med = r['m'], r['b']
    m_lo, m_hi   = np.percentile(flat[:, 0], [16, 84])
    b_lo, b_hi   = np.percentile(flat[:, 1], [16, 84])
...
for col in ['Gamma', 'log10Fluence', 'PhotonIndex', 'log10NH', 'log10PeakFlux',
            'T90Err', 'FluenceErr', 'PhotonIndexErr', 'PeakFluxErr']:
    final[col] = combined.get(col, np.nan)
...
fig2.suptitle(
    'Emcee calibration + projected optical-only GRBs  (relative err cut: err/value > 1)\n'
    'Blue = overlap (calibration); green = projected kept; red = projected removed',
    fontsize=10)

proj_params_plot = {
    'logFa': ('logFa',  'logFaErr',  'log10Fa',  'log10FaErr', fa_cut),
    'logTa': ('logT_a', 'logTaErr',  'log10Ta',  'log10TaErr', ta_cut),
    'Alpha': ('Alpha',  'AlphaErr',  'Alpha',    'AlphaErr',   alpha_cut),
    'Beta':  ('Beta',   'betaErr',   'Beta',     'BetaErr',    beta_cut),
}
