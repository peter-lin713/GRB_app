...
#' Predictor matrix fed to MICE: the 10 standard features PLUS the 4 native-
#' optical columns added alongside them (Alpha_opt/Beta_opt/log10Fa_opt/
#' log10Ta_opt -- real native-optical value for optical-only GRBs, structural
#' 0 for X-ray-native GRBs; never missing, so MICE passes them through
#' unchanged). Additive: the existing core columns are untouched.
features_for_mice_preds <- subset(raw_xray_data, select = c(
  log10T90, log10Fa, log10Ta, Alpha, Beta,
  log10Fa_opt, log10Ta_opt, Alpha_opt, Beta_opt, Gamma,
...
features_for_mice_errs <- data.frame(
  ...
  log10FaErr_opt   = raw_xray_data$log10FaErr_opt,
  log10TaErr_opt   = raw_xray_data$log10TaErr_opt,
  AlphaErr_opt     = raw_xray_data$AlphaErr_opt,
  BetaErr_opt      = raw_xray_data$BetaErr_opt,
...
if (do_mice) {
  ...
  # features_for_mice_errs hits "system is computationally singular" with
  # midastouch under every seed tried here (this dataset's *Err_opt columns
  # are identically 0 for all X-ray-only rows while simultaneously NaN for
  # the "shared" rows -- the v2 hard-zero-domain fix that leaves shared GRBs'
  # opt columns for MICE instead of hard-zeroing them -- combined with
  # T90Err's separate NaN block for optical-only rows, some internal
  # midastouch donor-regression subproblem is rank-deficient regardless of
  # seed). Dataset B (the original opt-cols-added run) never hit this because
  # it used the older v1 convention where shared GRBs' opt-err columns were
  # hard-zeroed too, i.e. never actually missing. Switching just this call to
  # 'pmm' (mice's other standard method) resolves it.
  mice_model_errs        <- mice(data = features_for_mice_errs, m = 20, maxit = 20, method = "pmm", printFlag = FALSE)
