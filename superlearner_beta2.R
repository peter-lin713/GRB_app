...
if (any(bad)) {
  ...
  features_for_mice_errs  <- features_for_mice_errs[!bad, ]
}


# ---- Impute ------------------------------------------------------------------
#' impute_method: "mice" (default, midastouch m=20) or "missforest" (single
#' pass, no generalization pool -- matches the plain missForest(...)$ximp call
#' spencer's actual best-result harness uses, for an apples-to-apples imputer
#' comparison against the same otherwise-unchanged pipeline). Env var:
#'   IMPUTE_METHOD=missforest Rscript superlearner_beta2.R <args...>
#' Errors are always MICE-imputed either way (unchanged).
impute_method <- tolower(Sys.getenv("IMPUTE_METHOD", "mice"))
cat("impute_method:", impute_method, "\n")
...
if (do_mice) {
  ...
  if (impute_method == "missforest") {
    require(missForest)
    set.seed(1)
    features_for_mice_preds <- missForest::missForest(features_for_mice_preds)$ximp
  } else {
