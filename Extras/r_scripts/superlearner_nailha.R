...
#source('SL.custom_caret.R')
# caret_analysis=T
# source('Custom_SL/SL.custom_caret.R')
# if(caret_analysis){
#   libs = c(
#     'SL.caret.rpart', 'SL.caret.ANFIS','SL.caret.brnn') # the 29 that work + GAM1
#   libnames<- '_CARET_'
# }

##### MAKING DIRECTORIES #####

PLOTaddr <-paste("Plot_Output_rf_xgb_brnn/") #THIS HOLDS THE ADDRESS AT WHICH THE PLOTS ARE OUTPUT
if(!dir.exists(PLOTaddr)){dir.create(PLOTaddr)}

###### CARET #####
tune_caret = list(
  #method = c('lmStepAIC', 'lm', 'null', 'rpart', 'glm.nb', 'nnls') #reported as working by nailha
  #method = c('logicBag', 'bagEarth', 'bagEarthGCV', 'bag', 'bartMachine', 'bayesglm')
  #method = c('gamboost', 'glmboost', 'BstSm', 'blackboost', 'bstTree', 'rpart1SE', 'rpart2')
  #method = c('cforest', 'ctree', 'ctree2', 'randomGLM', 'xgbDART', 'xgbLinear', 'xgbTree', 'elm')
  #method = c('gaussprLinear', 'gaussprRadial', 'gamLoess', 'bam', 'gam', 'krlsPoly') ##TOOK TOO LONG. NEED TO BE BROKEN UP
  #method = c('pcr', 'ppr', 'qrf', 'qrnn', 'rqlasso', 'krlsRadial', 'rbf', 'rbfDDA', 'ranger', 'Rborist')
  #
  #method = c('gam', 'bam') #TOOK EXTREMELY LONG
  #
  #method = c('gaussprLinear', 'gaussprRadial', 'krlsPoly') #don't work
  #method = c('bagEarth', 'bagEarthGCV', 'bayesglm', 'rpart1SE', 'rpart2', 'xgbDART', 'xgbLinear', 'xgbTree', 'ctree', 'ctree2', 'ranger', 'ppr', 'gamLoess')
  #
  #
  #
  #method = c('rf', 'extraTrees', 'rfRules')
  #method = c('RRF', 'relaxo', 'rvmLinear', 'rvmPoly', 'rvmRadial', 'ridge', 'foba', 'rlm', 'xyf', 'slps')
  #method = c('spikeslab') #ERRORED OUT
  #method = c('dnn', 'gbm', 'SBC', 'gamSpline')
  #method = c('glm', 'glmStepAIC', 'glmnet_h2o', 'glmnet')
  #method = c('gbm_h2o', 'kknn', 'knn', 'svmLinear3', 'logreg', 'avNNet', 'monmlp', 'mlp', 'mlpWeightDecay', 'mlpWeightDecayML')
  #method = c('mlpML', 'msaenet', 'mlpSGD', 'mlpKerasDropout', 'mlpKerasDecay', 'earth', 'gcvEarth', 'mxnet')
  #method = c('mxnetAdam', 'neuralnet', 'nnet', 'superpc','svmBoundrangeString', 'svmExpoString')
  #method = c('svmLinear', 'svmLinear2', 'svmPoly', 'svmRadial', 'svmRadialCost', 'svmRadialSigma', 'svmSpectrumString')
  #method = c('blasso', 'lasso', 'evtree', 'nodeHarvest', 'WM', 'pcaNNet', 'parRF', 'partDSA')
  #method = c('kernelpls', 'pls', 'simpls', 'widekernelpls', 'plsRglm') #ALL 5 ERROR OUT unless there's a 'good' model with them
  #method = c('rf', 'rlm', 'gbm', 'gamSpline', 'glm', 'glmStepAIC', 'glmnet', 'knn', 'avNNet', 'earth', 'gcvEarth', 'nnet', 'svmLinear2', 'pcaNNet')
  #
  method = c('lmStepAIC', 'lm', 'null', 'bayesglm', 'ranger', 'gamLoess', 'xgbDART', 'xgbLinear', 'xgbTree', 'ppr', 'bagEarthGCV', 'rf', 'gamSpline', 'glm', 'glmStepAIC', 'gcvEarth', 'knn', 'rlm', 'svmLinear2', 'glmnet', 'gbm')
  ,verboseIter = F, tuneLength = 1
)
caret_learner <- create.Learner('SL.caret',
                                tune = tune_caret,detailed_names = T)
libs=c(caret_learner$names)
libnames<- '_CARET_'
...
CVmodel<-foreach(j = 1:loop, .packages=c("SuperLearner", "caret" ,"xgboost", "randomForest", "gbm", "lattice", "latticeExtra", "Matrix", "glmnet", "biglasso","e1071",'earth','party'), 
  ...
  s9 <- CV.SuperLearner(Y = Response, X = Predictors,SL.library = libs,family = "gaussian",verbose=F)
  
  Algo_risk <- s9$cvRisk
  Algo_coeff <- coef(s9)
  
  #CVpred <- predict(s9,PredictionData)$pred
  
  return(list(1,Algo_coeff,s9$SL.predict,Algo_risk))
...
for (j in 1:loop) { # Iterate through the number of times SuperLearner was run to add each metric into a vector.
  ...
  co[j,] <- colMeans(CVmodel[[j]][[2]])
  #AlgoRisks[j,] <- colMeans(CVmodel[[j]][[4]])
...
save.image(file = paste0(PLOTaddr,'Image_file.Rdata'))
