...
#### function definition ####
StepAICParameter <- function(model,riks=T,tune,cv=T){
  #t=T
  #riks=T
  #print(slmodel)
  
  n_o_m <- length(tune$steps)*length(tune$direction)
  ...
  if (cv) {
    ...
    cvrisk <- matrix(data = model,nrow = length(tune$steps),ncol = length(tune$direction), byrow = F)
    mm<-'10fCV'
  }else{
    ...
    cvrisk <- matrix(data = model$cvRisk,nrow = length(tune$steps),ncol = length(tune$direction), byrow = F)
    mm<-'1SL'
  }
  
  #rownames(cvrisk)<-tune$ntrees
  rownames(cvrisk)<-tune$steps
  colnames(cvrisk)<- tune$direction
  ...
  plot(rownames(cvrisk),cvrisk[,1],type = 'l',col='red',xlab = 'Steps',ylab = yl,
...
if(StepAIC){
  ...
  #tune = list(ntrees = c(150),#800,900),#,500,600,700,800),
  #            max_depth = c(1:5),
  #            shrinkage = c((15:20)/1000))
  
  #tune = list(span = c(0.5, 0.6, 0.75, 0.8, 0.9),
            #  l.family = c("gaussian", "symmetric"))
  
  tune = list(direction = c("both",  "forward"),
                  steps = c(1,2,3,4,5,10,20, 30, 40, 50, 60, 70))
  
  learners = create.Learner("SL.stepAIC", tune = tune, detailed_names = T, name_prefix = "stepAIC")
  model_name="StepAIC"
}else{
  tune = list(mtry = c(2)
              , ntree = c(5:13)*50
...
#####WORKING UNTIL HERE
if(StepAIC){
  StepAICParameter(model,T, tune,F)
}else{
 StepAICParameter(model,T,tune,F)
}
...
if(StepAIC){
  StepAICHyperParameter(rowMeans(xgbrisk),T,tune)
  StepAICHyperParameter(rowMeans(xgbcorr),F,tune)
}else{
