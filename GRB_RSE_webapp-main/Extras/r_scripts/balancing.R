...
#GRBPred <- read.csv("grb_xray_imputed.csv", header = TRUE, row.names = 1) #commented out to use GRBPred in the parent script
...
GRBPred <- reimported[, -((ncol(reimported)-2):ncol(reimported))]
