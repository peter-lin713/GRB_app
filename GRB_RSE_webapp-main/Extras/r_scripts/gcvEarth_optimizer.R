...
#### function definition ####
EarthHyperParameter <- function(model,riks=T,tune,cv=T){
  #t=T
  #riks=T
  #print(slmodel)
  
  n_o_m <- length(tune$degree)
  
  
  if (cv) {
    #cvrisk <- matrix(data = model,nrow = length(tune$ntrees),ncol = length(tune$max_depth),byrow = F)
    cvrisk <- matrix(data = model,nrow = length(tune$degree),ncol = 1, byrow = F)
  ...
  png(filename = paste(model_name,'_Earth_plot',mm,'_.png',sep = ''),width=3*200,height=2*200)
  #plot(rownames(cvrisk),cvrisk[,1],type = 'l',col='red',xlab = 'Max Depth',ylab = yl,
  plot(rownames(cvrisk),cvrisk[,1],type = 'l',col='red',xlab = 'Degrees',ylab = yl,
  ...
  legend( #title = 'XGB depth',
    title = paste0(model_name,'degree'),
    pos,
    inset = 0.1,
...
png(filename = 'model_compare_plot.png',res=500,width=3000,height=3000)
par(mar=c(5, 10, 4, 2))
