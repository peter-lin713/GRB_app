...
def run_r_script(script_name, input_file, output_file=None, apply_mice=None, m_estimator=None, custom_models=None, m_est_weight=None, n_loops = None,
                 correlation_cutoff=None, RMSE_cutoff=None):
    """
...
task = st.sidebar.radio(
    "Choose a task:",
    ("**Redshift Estimator**","**MICE Missing Features**", "**GAM/GLM Formula Generator**", "**Superlearner**", "**Model Comparison**", "**Relative Importance**", "**Other task**", "**Documentation**")
)
...
if task == "**Redshift Estimator**":
    ...
    if option == '**Upload a file**':
        ...
        if uploaded_file is not None and process_file:
            ...
            plot_names = plot_names = ["Box plot for each GRB with error bars shown for each prediction", 
...
#MICE Missing
elif task == "**MICE Missing Features**":
...
# Superlearner
    ...
    with sl_row2[0]: m_estimator = st.checkbox("**Apply M Estimator**")
    m_est_weight = 0.5
    if m_estimator:
        with sl_row2[1]: m_est_weight = st.text_input("**Custom Weight Threshold**", value = 0.5, max_chars=20)
    with sl_row3[0]: custom_models = st.checkbox("**Custom Models**")
    # List of 16 models
    models = [
    ...
    if uploaded_file is not None and process_file:
        ...
        # Run the (yet to be created) R script for Superlearner
        #run_r_script("new_standalone_superlearner.R", temp_input_file, apply_mice=str(apply_mice), m_estimator=str(m_estimator), custom_models=str(custom_models), m_est_weight=m_est_weight)
        run_r_script("superlearner.R", temp_input_file, apply_mice=str(apply_mice), m_estimator=str(m_estimator), custom_models=str(custom_models), m_est_weight=m_est_weight, n_loops=100)
        ...
        clean_up(temp_input_file) #, output_file)
        superlearner_plots = [ 'Plot_Output10fCV_plot_OG__OG_1GAM_1GLM_2algo_17vrb_20times.png',
                            'Plot_Output10fCV_plot_redshift_OG__OG_1GAM_1GLM_2algo_17vrb_20times.png',
                            'Plot_OutputDeltaZSpreadBLL.png',
                            'Plot_OutputNormDeltaZSpreadBLL.png'
                            ]
        if remove_catout:
            superlearner_plots = ['Plot_Output10fCV_plot_redshift_removing_CatOutl_ _OG_1GAM_1GLM_2algo_17vrb_20times.png',
                            'Plot_Output10fCV_plot_removing_CatOutl_ _OG_1GAM_1GLM_2algo_17vrb_20times.png'] + superlearner_plots[2:]
        if m_estimator:
            superlearner_plots.append('M_estimator_weights_after.png')
        plot_names=["Box Plot depicting predicted vs observed Log_10 of Redshift(Z)","Box Plot depicting predicted vs observed Redshift(Z)","Histogram depicting frequency vs change in redshifts","Histogram depicting frequency vs change in normalized redshifts"]    
...
# Get New Formulas
    ...
    correlation_cutoff = st.slider("**Choose correlation quantile cut-off**", 0, 100, value=99)  # 3 is the default value 
    RMSE_cutoff = st.slider("**Choose RMSE quantile cut-off**", 0, 100, value=3)  # 3 is the default value 
    ...
    if uploaded_file is not None and process_file:
        ...
        # Run the R Script
        run_r_script("Formula_Search_Aditya.R", temp_input_file, output_file2, correlation_cutoff=correlation_cutoff/100, RMSE_cutoff=RMSE_cutoff/100)
...
# Relative Importance
    ...
    if uploaded_file is not None and process_file:
        ...
        clean_up(temp_input_file) #, output_file)
        Importance_plots = [ 'Plot_OutputRelinf_SelectiveVariables_removing_CatOutl_ _OG_1GAM_1GLM_2algo_17vrb_20times.png'
                            ]
