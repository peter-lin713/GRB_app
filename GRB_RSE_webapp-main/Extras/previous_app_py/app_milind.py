...
def run_r_script(script_name, input_file, output_file=None, apply_mice=None, m_estimator=None, remove_catout=None, m_est_weight=None):
    """
    ...
    if remove_catout is not None:
        command.append(str(remove_catout))
...
task = st.sidebar.radio(
    "Choose a task:",
    ("Redshift Estimator", "Superlearner", "Get New Formulas", "Relative Importance", "Other task", "Documentation")
)
...
# Superlearner
    ...
    with sl_row2[0]: m_estimator = st.checkbox("Apply m_estimator")
    m_est_weight = 0.5
    if m_estimator:
    ...
    if uploaded_file is not None and process_file:
        ...
        run_r_script("new_standalone_superlearner.R", temp_input_file, apply_mice=str(apply_mice), m_estimator=str(m_estimator), remove_catout=str(remove_catout), m_est_weight=m_est_weight)
...
# Get New Formulas
    ...
    if uploaded_file is not None and process_file:
        ...
        output_file = "new_formulas.txt"  # Output file name
        # Run the R Script
        run_r_script("Formula_Search.R", temp_input_file, output_file)
        ...
        if os.path.exists(output_file):
            ...
            new_column_names = {
                         '': 'Sl',
                         'Formula': 'Formulae',
                         'LinearCorrel': 'Linear Correlation',
                         'LinearRMSE': 'Linear Root Mean Squared Error (RMSE)',
                        # Add more mappings as needed
                         }
