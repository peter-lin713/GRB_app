    ...
    if option == '**Upload a file**':
        ...
        if uploaded_file is not None and process_file:
            ...
            plot_names = ["Box plot for each GRB with error bars shown for each prediction",
                                       ...
                                       "Box Plot for Bias Corrected predictions",
                                       "Histogram comparing Redshift got training set and Generalization Set"]
            # Display plots generated by R script
            plot_dir = ""
            redshift_plots = [ "boxplot.png", "log10z_histogram.png", "z_histogram.png","Redshift_boxplot.png","Generalization_set_redshift_distributions.png"]
...
#Data Visualisation
    ...
    training_set = st.checkbox("**Select this checkbox if you use the training set?**")
    ...
    if uploaded_file is not None and process_file:
        ...
        if Scatter == True and  Mice == True:
            ...
            with open(temp_input_file, "wb") as f:
                f.write(uploaded_file.getbuffer())
            run_r_script("Data_Visualisation.R", temp_input_file, str(training_set))
...
# Compare Models
    ...
    st.write("**Please upload training data (Note: The model trained here can also be used to get redshift predictions for Generalization set in Redshift Estimator by selecting Custom Models)**")
    apply_mice = st.checkbox("**Apply MICE**")
    ...
    if uploaded_file is not None and process_file:
        ...
        #output_file = "output_superlearner.txt"  # Output file name
        # Run the (yet to be created) R script for Superlearner
        run_r_script("model_comparison.R", temp_input_file, apply_mice=str(apply_mice),upsampling='false', m_estimator='false', custom_models='true', m_est_weight=0.5, n_loops=n_loops)
        #run_r_script("superlearner_extended.R", temp_input_file, apply_mice=str(apply_mice), n_loops=n_loops)

        clean_up(temp_input_file) #, output_file)
        plot_dir = "Plot_Output"
        superlearner_plots = [ 'Plot_Output/model_compare_plot.png']
        #superlearner_plots = [os.path.join(plot_dir, plot_name) for plot_name in superlearner_plots]
        #superlearner_plots = ["z_pred_v_obs_linear_with_catOutl_correlation_plot.png"," model_compare_plot.png","LassoFeatures.png"]
        plot_names=["Weights assigned by SuperLearner for Different Models"]    
        for i, plot_file in enumerate(superlearner_plots):
            if os.path.exists(plot_file):
                image = Image.open(plot_file)
                st.image(image, caption=plot_names[i])
...
# Get New Formulas
    ...
    uploaded_file = st.file_uploader("**Provide Training Data (.csv or .txt)**", type=['csv', 'txt'])
    process_file = st.button("**Submit**")
...
# Relative Importance
    ...
    if uploaded_file is not None and process_file:
        ...
        #output_file = "output_superlearner.txt"  # Output file name
        # Run the (yet to be created) R script for Superlearner
        run_r_script("Influence_Calculation_v3.R", temp_input_file, str(supmodel) )


        clean_up(temp_input_file) #, output_file)
        plot_dir = "Testing_webapp_RelativeInfluence"
        Importance_plots = [ 'Relinf_SelectiveVariables.png']
