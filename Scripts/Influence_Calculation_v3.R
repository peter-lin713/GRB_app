...
if(run_locally){
  ...
  supmodel <- as.logical(tolower(args[2]) == "true")
  upsampling = F
  do_m_estimator = T
  custom_models = T
  weight_threshold = 0.65
...
addr <-paste("Testing_webapp_RelativeInfluence/") #THIS HOLDS THE ADDRESS AT WHICH THE FILES ARE OUTPUT
PLOTaddr <-paste("Testing_webapp_RelativeInfluence/") #THIS HOLDS THE ADDRESS AT WHICH THE PLOTS ARE OUTPUT
...
#SuperLearner(Y = Response,X = Predictors,family = gaussian(),SL.library = libs)
#inflFunc(Predictors,model)
...
plotnames<- "influence_plots"
system.time({
  #TrainData$LabelNo<-as.numeric(TrainData$LabelNo)
  
  
  if(supmodel == "true") {
    model=readRDS("superlearner_model")
  } else {
    model <- SuperLearner(Y = Response, X = Predictors, family = gaussian(), SL.library = libs)
  }
  print(libs)
  #sl_model=SuperLearner(Y = Response, X = Predictors,family = gaussian(), SL.library = libs,verbose = F)
  #saveRDS(s_model, file = "superlearner_model")
  
   #To train model accurately
 ...
 # coef(model)
  ...
  png(filename = paste(PLOTaddr,"Relinf_SelectiveVariables.png",sep=""),width = 1000*sz,height = 1000*sz,res = 160)
  par(mar=c(5,8,0,1)+.1)
