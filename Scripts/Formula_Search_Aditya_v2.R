...
if(run_locally){
  ...
  input_file <- args[1]
  output_file <- args[2]
  glmb <- as.logical(tolower(args[3]) == "true")
  correlation_cutoff = as.numeric(args[4])
  RMSE_cutoff = as.numeric(args[5])
...
PLOTaddr <-paste("Plot_Output_testing/") #THIS HOLDS THE ADDRESS AT WHICH THE PLOTS ARE OUTPUT
...
manual_cv <- foreach(j = 1:length(all_formula)
  ...
  if(glmb)
  {gam_model <- glm(formula = gam_formula
                         ,data = GamTrainData
                         ,family = gaussian())
  #gam_model <- MASS::rlm(gam_formula,train_set,method = 'M')
