...
if(run_locally){
  ...
  # Using the OT baseline combined dataset instead of the original
  # combined_data_with_redshift.csv -- see README.md in this folder.
  raw_xray_data <- read.csv("Data/superlearner_training_ot_v3_errcut_relative.csv", header = TRUE, row.names = 1)
} else {
...
# Pick out Long GRBs
# The OT baseline dataset only ships log10T90 (no raw linear T90 column),
# so filter/derive directly in log space: log10T90 > 0.301 == T90 > 2s,
# same threshold and convention used elsewhere in this pipeline (superlearner.R).
...
MyLaptop = T   # this machine has no MPI runtime -- see README.md in this folder
